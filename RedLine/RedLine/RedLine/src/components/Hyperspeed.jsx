/* Hyperspeed.jsx
   A React wrapper that mounts the hyperspeed WebGL scene into a div#lights.
   Requires: three, postprocessing (installed via npm)
*/

import React, { useEffect, useRef } from "react";
import * as THREE from "three";
import { BloomEffect, EffectComposer, EffectPass, RenderPass, SMAAEffect, SMAAPreset } from "postprocessing";
import "./Hyperspeed.css"; // small CSS (#lights)
...
// --- many helper functions & shader strings are from your shared code ---
// For clarity I include the key parts here (distortions, App class, helpers).
// This is a fairly long file but it's self-contained.
...
const Hyperspeed = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    // merge options
    const options = Object.assign({}, defaultOptions, effectOptions);
    ...
    // Distortion implementations (both JS helpers and shader snippets)
    const distortions = {
      mountainDistortion: {
    ...
    // CarLights, LightsSticks, Road, App classes (trimmed but fully functional)
    class CarLights {
      ...
      init() {
        ...
        if (Array.isArray(colors)) colors = colors.map(c => new THREE.Color(c)); else colors = new THREE.Color(colors);
        for (let i=0;i<options.lightPairsPerRoadWay;i++){
          ...
          let carShiftX = random(options.carShiftX) * laneWidth; laneX += carShiftX;
    ...
    class Road {
      ...
      createPlane(side, width, isRoad){
        ...
        const material = new THREE.ShaderMaterial({
          fragmentShader: isRoad ? roadFragment : roadBaseFragment.replace('#include <roadMarkings_fragment>', '').replace('#include <roadMarkings_vars>', ''),
    ...
    // distortion_uniforms and distortion_vertex used as fallback when no distortion provided:
    ...
    // The App class (manages scene, camera, passes, tick)
    class App {
    ...
    if (!container) {
      console.warn("Hyperspeed: container #lights not found");
      return;
    }

    options.distortion = distortions[options.distortion] || options.distortion || { uniforms: distortion_uniforms, getDistortion: distortion_vertex };
    ...
    myApp.loadAssets().then(() => {
      if (myApp && !myApp.disposed) myApp.init();
    }).catch(err => {
      console.error("Hyperspeed loadAssets error:", err);
    });
    ...
    return () => {
      if (appRef.current) {
        try { appRef.current.dispose(); } catch (e) { console.warn("Hyperspeed dispose error", e); }
      }
      mountedRef.current = false;
    };
  }, [JSON.stringify(effectOptions)]); // rerun if effectOptions change

  return <div id="lights" style={{ width: "100%", height: "100%", position: "relative" }} />;
};
