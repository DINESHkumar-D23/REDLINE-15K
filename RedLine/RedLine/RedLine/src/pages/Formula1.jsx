// src/pages/Formula1.jsx
import React, { useEffect, useRef, useState, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import { motion } from "framer-motion";
import Header from "../components/Header";
import "../css/Formula1.css";
import { tracks as importedTracks } from "../data/tracks.js";

export default function Formula1() {
  const navigate = useNavigate();

  // duplicate tracks for seamless loop
  const loopTracks = [...importedTracks, ...importedTracks];

  // Regulation state (persisted)
  ...
  const selectedTrack = importedTracks.find((t) => t.id === selectedTrackId) || null;
  ...
  const onBegin = () => {
    ...
    const track = importedTracks.find((t) => t.id === selectedTrackId);
    if (!track) return;
      ...
      <main style={{ paddingTop: 92 }}>
        <section className="f1-page">
          ...
          <section className="regulations-section top">
            ...
            >
              <h3 id="reg-title">{regulation === "2022" ? "2022 Regulations (legacy)" : "2026 Regulations (updated)"}</h3>
              <p className="reg-summary">
                {regulation === "2022"
                  ? "The 2022 regulations introduced ground-effect aerodynamics, larger 18-inch wheels, and a focus on closer racing."
                  : "The 2026 regulations further evolve the championship: powertrain electrification targets, clearer aero parity, and updated safety standards."}
              </p>
              <details className="reg-details">
                <summary>Read more</summary>
                <p>
                  {regulation === "2022"
                    ? "Key points: simplified front wings, tighter aero around the car, and reworked floor tunnels."
                    : "Key points: hybrid energy deployment changes, updated crash structure limits, refined aero surfaces for closer following."}
                </p>
              </details>
            </motion.article>
          </section>
          ...
          >
            ...
            >
              {loopTracks.map((t, i) => {
                ...
                // Ensure stats exist — fallback to '-' if not present
          ...
          <div className="simulation-cta">
            <div className="selection-summary" aria-live="polite">
              <div><strong>Selected Regulation:</strong> {regulation === "2022" ? "2022 (legacy)" : "2026 (updated)"}</div>
              <div><strong>Selected Track:</strong> {selectedTrack ? selectedTrack.name : "None"}</div>
            </div>
